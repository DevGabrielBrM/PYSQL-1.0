...
class GUI():
    ...
    # Para criar uma janela *obs: PYSQL será o nome fantasia da aplicação
    window = Tk()
    ...
    # Criar o cabeçalho da Listbox
    listHeader = Label(window, text="ID       Nome                 Sobrenome               Email                        CPF", bg='grey', anchor='w')
    ...
    listHeader.grid(row=0, column=2, columnspan=2, sticky='w')  # Cabeçalho
    listClientes.grid(row=1, column=2, rowspan=10)
    scrollClientes.grid(row=1, column=3, rowspan=10, sticky='NS')
