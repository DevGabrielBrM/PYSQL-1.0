...
def delete(id):
    ...
    trans.execute("VACUUM")
    trans.execute("SELECT id FROM clientes ORDER BY id")
    rows = trans.fetchall()
    for new_id, (old_id,) in enumerate(rows, start=1):
        trans.execute("UPDATE clientes SET id=? WHERE id=?", (new_id, old_id))
    trans.execute("UPDATE sqlite_sequence SET seq = (SELECT MAX(id) FROM clientes) WHERE name = 'clientes'")
    trans.persist()
